...
"""copy(), deepcopy()"""
# copy() создаёт поверхностную (shallow) копию объекта. В копии создаётся новый объект, но вложенные объекты (например,
# списки внутри списка) остаются общими.

# deepcopy() создаёт глубокую копию объекта, рекурсивно копируя все вложенные объекты, чтобы изменения в копии не влияли на оригинал.
...
# .upper()
# .capitalize()
# .title() # каждое новое слово с заглавной буквы
...
# .join() # объединяет элементы итерируемого объекта в строку, используя текущую строку как разделитель
','.join(['a', 'b', 'c']) # a,b,c

# .find() # возвращает индекс первого вхождения подстроки или -1, если индекс не найден
string.find('e') # 1

# .index() # как find(), но вызывает ошибку, если подстрока не найдена

# .replace(old, new[, count]) - заменяет в строке подстроку old на new
"hello world".replace('world', 'python')

# .startswith()
string.startswith("he") # True

# .endswith()
string.endswith("lo") # True
...
# .zfill(width) - дополняет строку нулями слева до заданной длины
"42".zfill(5) # 00042

# .count() - возвращает количество вхождений подстроки
string.count('l') # 2

# .partition(sep) - разбивает строку на кортеж из трёх частей:
...
"""Методы множества"""
# Множества - НЕУПОРЯДОЧЕННАЯ ИЗМЕНЯЕМАЯ коллекция уникальных элементов
