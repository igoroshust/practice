...
# zip() - объединяет несколько итерируемых объектов (например, списков или кортежей) в один итератор, который генерирует кортежи, содержащие
# элементы из каждого из переданных итерируемых объектов. Она останавливается, когда достигает конца самого короткого итерируемого объекта.
# list1 = [1, 2, 3]
# list2 = ['a', 'b', 'c']
#
...
# items() - метод используется для получения пар ключ-значение из словаря. Он возвращает представление в виде итератора, который генерирует кортежи,
...
# # map() - применяет заданную функцию к каждому элементу итерируемого объекта (например, списка) и возвращает итератор с результатами. Это позволяет легко перебирать данные.
# numbers = [1, 2, 3, 4]
# def square(x):
#     return x * x
#
...
# filter() - применяет заданнную функцию к каждому элементу итерируемого объекта и возвращает итератор, содержащий только те элменты, для которых функция вернула True.
