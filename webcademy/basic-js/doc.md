# Темы для разбора:
- Способ вставки контента в элемент
- Рекомендации по выбору метода поиска объектов DOM
- Замыкания
- Приватные переменные
- IIFE
- Приватное поле полей классов
- Механизм деструктуризации массива
- async/await
- поллинг данных через setInterval
- XMLHttpRequest
- Подробно о том, как браузер читает скриптовые файлы (два раза + ответ на вопрос, почему FD можно вызывать до объявления)

- event loop
- ASI - Automatic Semicolon Insertion (автоматическая подставка точек с запятыми)
- Однопоточность в JS
- stopPropagation
- preventDefault
- event 
- target
- Событийная модель JavaScript (DOM Events -- event, target, etc.)
- defer
- Инверсия контроля (функция высшего порядка контролирует вызов коллбека)

- Кастомные события `modal.dispatchEvent(new Event('modal:open'));`
- classList
- closest
- FETCH API
- DOM API
- DOMTokenList
- Поднятие (hoisting)
- Полифилы
- Сферы применения ЯП (когда лучше использовать один, когда - другой);
- this
- throw
- ; - подробнее о назначении точки с запятой
- Замыкания
- XMLHttpRequest
- Server-Side Rendering
- Функции высшего порядка
- Выражения, инструкции
- Принцип определения объёма необходимой памяти
- Метрики оценки качества веб-приложения
- Тестирование во фронтенде
- Использование постман
- SASS, LESS, SCSS
- jQuery
- document
- querySelector
-  Airbnb JS Style Guide


### Коллбеки
Коллбеки (callbacks) - это функции, которые передаются в качестве аргументов другим функциям и вызываются внутри них. Это один из способов обработки асинхронных операций в JavaScript, таких как запросы к серверу, таймеры или события. Функция, которая принимает коллбек, называется функцией высшего порядка (higher-order function).

Коллбеки позволяют выполнять код после завершения какой-то операции, не блокируя основной поток выполнения. Однако они могут привести к коллбек-хеллу, когда коллбеки вложены глубоко, что делает код трудночитаемым.

**Основные понятия**
- Синхронные коллбеки: вызываются сразу;
- Асинхронные коллбеки: вызываются позже, например, после таймера или HTTP-запроса;
- Анонимные коллбеки: определяются прямо в вызове функции `setTimeout(() => console.log('Done'), 1000)`
- Именованные коллбеки: определяются отдельно.

#### Примеры коллбеков
**1. Простой синхронный коллбек**
```javascript
function processData(data, callback) {
    // Функция высшего порядка
    console.log('Обработка данных:', data);
    callback(data.toUpperCase());
}

function displayResult(result) {
    // Коллбек-функция
    console.log('Результат:', result);
}

processData('hello', displayResult);  // Вывод: Обработка данных: hello, Результат: HELLO
```

Мой пример
```javascript
function processData(data, callback) {
    console.log('Обработка данных: ', data);
    console.log(
        'Данные в результате обработки: ', callback(data)
    );
}

function upperData(data) {
    return data.toUpperCase();
}

function lowerData(data) {
    return data.toLowerCase();
}

processData('Hello', upperData);
processData('Hello', lowerData);
```

**2. Асинхронный коллбек с таймером**
```javascript
function delayedGreeting(name, callback) {
    setTimeout(() => {
        console.log(`Привет, ${name}`);
        callback();
    }, 2000);
}

delayedGreeting('Алиса', () => console.log('Коллбек выполнен после задержки (2 секунды)'));
```

Асинхронные коллбеки с таймером нужны для имитации или выполнения операций, которые не должны блокировать основной поток выполнения JS. JS однопоточный, и синхронные операции (например, другие циклы) могут "заморозить" интерфейс. Таймеры позволяют запускать код с задержкой, не останавливая основную программу.

Применение: 
- Имитация реальных асинхронных задач: загрузка данных, анимации, опрос сервера.
- Неблокирующее выполнение: код продолжает работать, пока таймер "тикает".
- Управление временем: например, для повторяющихся задач или задержек в UI.

Дополнительный пример (полезно в браузерах для анимаций или в Node.js для таймеров, но для сложных асинхронных задач лучше использовать промисы или async/await)
```javascript
function delayedTask(taskName, delay, callback) {
    console.log(`Запуск задачи: "${taskName}"`);
    setTimeout(() => {
        console.log(`Задача "${taskName}" завершена`);
        callback(); // Коллбек вызывается после задержки
    }, delay);
}

delayedTask('Загрузка данных', 3000, () => console.log('Теперь можно обновить UI'));
```

**3. Коллбек в массивах (методы вроде `forEach`, `map`)**
```javascript
const numbers = [1, 2, 3];
numbers.forEach((num) => console.log(num * 2)); // forEach принимает коллбек и применяет его к каждому элементу
```

**4. Пример с ошибками (обработка ошибок в коллбеках)**
```javascript
function fetchData(url, successCallback, errorCallback) {
    // Имитация асинхронного запроса
    setTimeout(function(){
        if (url == 'valid') {
            successCallback('Данные успешно обработаны');
        } else {
            errorCallback('Ошибка URL');
        }
    }, 1000);
}

fetchData('valid', 
    (success) => console.log(success),
    (error) => console.log(error)
);
```

**Преимущества и недостатки коллбеков**
- Примущества: простота, поддержка асинхронности
- Недостатки: коллбек-хелл, сложность отладки, инверсия контроля (функция высшего порядка контролирует, когда вызывать коллбек).


#### Использование коллбеков в веб-разработке
Коллбеки широко используются в веб-разработке для обработки асинхронных операций (пользовательские взаимодействия, запросы к серверу или анимации). Они позволяют выполнять код после завершения задачи, не блокируя основной поток (например, UI браузера).

**Примеры**
1. Загрузка данных из API с обработкой ошибок. Например, при клике на кнопку происходит загрузка данных о пользователе с сервера. Мы используем коллбек для обработки успешного ответа или ошибки (в реальности лучше использовать async/await, но коллбеки - классический подход, особенно в старых библиотеках вроде jQuery). Коллбек полезен, так как позволяет обрабатывать результат запроса после его завершения, без необходимости останавливать весь код.

2. Обработка событий DOM (самый распространённый случай):
- `addEventListener` для кликов, скролла, загрузки страницы. События происходят асинхронно, коллбек выполняется при наступлении события.
```javascript
button.addEventListener('click', () => console.log('Клик!'));
```

3. Асинхронные запросы к серверу (AJAX)

4. Таймеры и анимации
- `setTimeout`, `setInterval`, `requestAnimationFrame`. Код выполняется с задержкой или в цикле, не блокируя UI.

5. Работа с массивами и коллекциями.
- `forEach`, `map`, `filter`, `reduce`. Синхронная итерация, коллбек применяется к каждому элементу.

6. Библиотеки и фреймворки:
- В jQuery (коллбеки для анимаций), Node.js или кастомные модули. Используются для обратной совместимости или простоты в небольших проектах.


**Принцип работы коллбеков в функциях высшего порядка `addEventListener`, `setTimeout` и т.д.**
`addEventListener`, `setTimeout`, `setInterval`, `requestAnimationFrame` - это все функции высшего порядка (higher-order functions). Они принимают другую функцию (коллбек) как аргумент и вызывают её внутри себя в определённый момент времени. Коллбек передаётся в скобках как параметр (например, `setTimeout(() => console.log('Done'), 1000)`), и он выполняется асинхронно, когда наступает нужное условие (событие, задержка, кадр анимации и т.д.).

Это классический паттерн коллбеков в JavaScript: функция высшего порядка контролирует, КОГДА и КАК вызывать коллбек, позволяя обрабатывать асинхронные операции без блокировки основного потока.
Это классический паттерн коллбеков в JavaScript: функция высшего порядка контролирует, КОГДА и КАК вызывать коллбек, позволяя обрабатывать асинхронные операции без блокировки основного потока.


#### Коллбек-хелл
Коллбек-хелл - это антипаттерн, когда асинхронные коллбеки вложены глубоко друг в друга, образуя "пирамиду" из функций. Это делает код нечитаемым, трудно отлаживаемым и подверженным ошибкам.

**Проблемы**
- Читаемость. Код смещается вправо, сложно понять последовательность.
- Обработка ошибок. Ошибки нужно обрабатывать на каждом уровне.
- Управление. Трудно добавить/изменить логику или отменить операции.
- Возникает при зависимых асинхронных операциях (запросы к API, где результат одногоо нужен для следующего).

**Пример коллбек-хелла**
```javascript
function asyncOp1(data, callback) {
    setTimeout(() => callback(data + ' шаг-1'), 1000);
}
function asyncOp2(data, callback) {
    setTimeout(() => callback(data + ' шаг-2'), 1000);
}
function asyncOp3(data, callback) {
    setTimeout(() => callback(data + ' шаг-3'), 1000);
}

asyncOp1('Начало', (result1) => {
    console.log(result1);
    asyncOp2(result1, (result2) => {
        console.log(result2);
        asyncOp3(result2, (result3) => {
            console.log(result3); // Глубокая вложенность
        });
    });
});
```

**Как избежать?**
- 1. Промисы: цепочка `.then()` вместо вложенности
```javascript
function asyncOp1(data){
    return new Promise((resolve, reject) => {
        setTimeout(() => {
            const message = `${data} шаг-1`;
            resolve(message); // Передаём результат
        }, 1000);
    });
}
function asyncOp2(data){
    return new Promise((resolve, reject) => {
        setTimeout(() => {
            const message = `${data} шаг-2`;
            resolve(message);
        }, 1000);
    });
}
function asyncOp3(data){
    return new Promise((resolve, reject) => {
        setTimeout(() => {
            const message = `${data} шаг-3`;
            resolve(message);
        }, 1000);
    });
}

// Работа цепочки
asyncOp1('Начало')
    .then(result1 => {
        console.log(result1); // Вывод: Начало шаг-1
        return asyncOp2(result1);
    })
    .then(result2 => {
        console.log(result2); // Вывод: Начало шаг-1 шаг-2
        return asyncOp3(result2);
    })
    .then(result3 => {
        console.log(result3); // Вывод: Начало шаг-1 шаг-2 шаг-3
    })
    .catch(error => console.error('Ошибка:', error));
```


- 2. async/await (код будет более линейным, без вложенности + автоматическая обработка промисов)
```javascript
function asyncOp1(data) {
    return new Promise((resolve, reject) => {
        setTimeout(() => {
            const message = `${data} шаг-1`;
            resolve(message);  // Передаём результат
        }, 1000);
    });
}

function asyncOp2(data) {
    return new Promise((resolve, reject) => {
        setTimeout(() => {
            const message = `${data} шаг-2`;
            resolve(message);
        }, 1000);
    });
}

function asyncOp3(data) {
    return new Promise((resolve, reject) => {
        setTimeout(() => {
            const message = `${data} шаг-3`;
            resolve(message);
        }, 1000);
    });
}

async function runOperations(){
    try {
        const result1 = await asyncOp1('Начало');
        console.log(result1);
        const result2 = await asyncOp2(result1);
        console.log(result2);
        const result3 = await asyncOp3(result2);
        console.log(result3);
    } catch (error) {
        console.error('Ошибка: ', error);
    }
}

runOperations();
```

**Заключение**
Коллбек-хелл решается рефакторингом в более современные подходы, сохраняя асинхронность без потери производительности.








### Когда ставить ';' в JS
В JS `;` используются для разделения инструкций (statements). JS имеет автоматическую вставку точек с запятой (ASI - Automatic Semicolon Insertion), которая добавляет их в конце строк автоматически, но это не всегда надёжно. Рекомендуется ставить ';' для consistency (последовательности), ихюежания багов и следования стандартам (Airbnb style guide или ESLint). ASI может ошибаться, особенно в сжатом коде или при цепочках.

#### Когда нужно ставить ';'
1. После выражений (expressions), которые не являются частью блока:
- Присваивание: `let x = 5;`
- Вызовы функции: `console.log('Hello');`
- Return: `return value;`
- Throw: `throw new Error('Oops');`
- Break/continue: `break;`

2. После объявлений переменных: `const a = 1; let b = 2;`
3. После объявления функций: `function foo() {...}`
4. В цепочках `a = b; c = d` (чтобы избежать путаницы);
5. После IIFE (Immediately Invoked Function Expression): `(function() { ... })();`

#### Когда НЕ НУЖНО ставить ';'
1. После блоков кода (statements с фигурными скобками):
- Объявления функций: `function foo(){...}` (ASI добавит)
- if/else: `if (true) { ... }`
- for/while: `for (let i = 0; i < 10; i++) {...}`
- try/catch: `try {...} catch {...}`

2. После ключевых слов, открывающих блоки: `if`, `for`, `while`, `function`, etc.
3. В объектах/массивах: `const obj = { key: 'value' }`
4. После стрелочных функций без тела: `const foo = () => 'bar';` (здесь `;` нужен)



### Что работает быстрее: getElementByID или getElementsByClassName?
`getElementById` обычно быстрее `getElementsByClassName`, так как поиск по ID выполняется за константное время `(O(1))`, а поиск по классу требует перебора элементов `(O(n))`. Однако разница в производительности заметна только при массовом использовании методов в циклах. Для единичных операций разница незначительна.

#### Рекомендации по выбору метода
- Используйте `getElementById`, если нужен быстрый доступ к уникальному элементу.
- Используйте `getElementsByClassName`, если требуется работать с группой элементов, имеющих общий класс.
- Используйте `querySelector`, если нужен гибкий поиск по сложным селекторам вроде `#id.class` или `tag[attr="value"]`.

**Оптимизация**
Для оптимизации производительности в больших проектах можно комбинировать методы: сначала сузить область поиска с помощью `getElementById`, а затем использовать `getElementsByClassName` или `querySelector` внутри найденного контейнера.
Важно: в современных браузерах разница в производительности между этими методами часто незначительна, а выбор должен определяться читаемостью кода и удобством разработки, а не микрооптимизациями.





**querySelector ищет быстрее классы или ID?**
`querySelector` с селектором по ID работает быстрее, чем с селектором по классу, так как ID уникален и поиск выполняется быстрее. Но оба варианта медленнее, чем `getElementById` и `getElementsByClassName`.


### Как лучше вставлять контент в элемент?
Выбор способа вставки зависит от типа контента, безопасности и производительности. Вот основные методы:
- **innerHTML**: Вставляет HTML-строку (парсит теги). Быстрый, но уязвим к XSS. Используется для динамического HTML;
- **innerText**: Вставляет только текст (игнорирует HTML-теги, отображает как текст). Безопаснее, но медленнее (перерисовывает DOM). Подходит для простого текста без форматирования;
- **textContent**: Аналог innerText, но включает скрытый текст (например, в `<span style="display: none;">`). Быстрее innerText, не парсит HTML. Идеален для чистого текста.;
- **insertAdjacentHTML**: Вставляет HTML рядом с элементом (позиции: 'beforebegin', 'afterbegin', 'beforeend', 'afterend'). Безопаснее innerHTML, так как не заменяет весь контент, а добавляет. Подходит для фрагментарных вставок.

#### Какой способ выбрать?
- Для текста без HTML: `textContent` (быстрее и безопаснее innerText);
- Для HTML-контента: `insertAdjacentHTML` (меньше риска XSS, чем innerHTML);
- Для полной замены: `innerHTML` (если уверены в безопасности);
- Избегайте innerHTML с пользовательским вводом - используйте DOM-методы (createElement, appendChild) для максимальной безопасности. Тестируйте производительность на больших DOM.




### closest
`closest()` - метод DOM-элементов в JS, позволяющий найти ближайший родительский элемент, соответствующий заданному CSS-селектору. Поиск начинается с самого элемента и движется вверх по DOM-дереву до корня документа.
```javascript
element.closest(selector);

element - DOM-элемент, от которого начинается поиск
selector - CSS-селектор для поиска родительского элемента
```

**Принцип работы**
Метод последовательно проверяет:
1. Сам элемент на соответствие селектору;
2. Родительский элемент;
3. Далее всех предков по цепочке.

Как только находится первый подходящий элемент - поиск прекращается и возвращается найденный элемент. Если совпадений нет - возвращается null.

**Пример**
```html
<div>
    <p>Первый абзац</p>
    <div>
        <span>Это спан</span>
    </div>
<div>

<script>
const span = document.querySelector('span');
const div = span.closest('div');
</script>
```


### event, target, event.target
event, target, event.target - это концепции из событийной модели JavaScipt (DOM Events). Когда происходит событие (например, клик), браузер создаёт объект `event` и передаёт его в обработчик функции.

- `event`: Это объект события (типа `MouseEvent` или `Event`). Он содержит информацию о событии: координаты, тип, время и т.д. В коде `addEventListener('click', (event) => { ... })` event - это параметр, который можно назвать как угодно (например, `e`). event - объект, содержащий информацию по конкретному событию.

- `target`: Это свойство объекта event. Оно указывает на DOM-элемент, который вызвал событие (на котором произошёл клик). Если кликнули на `<li>`, `event-target` - это тот `<li>`.

- `event.target`: Полный доступ к элементу-источнику. Может быть использован для проверки, что клик был именно по целевому элементу, а не на пустом месте.


**Пример**
```javascript
tabList.addEventListener('click', (event) => {
    console.log(event.target); // Выведет элемент, на который кликнули (например, <li>)
    console.log(event.type); // 'click'
})
```




### className
`className` - это свойство DOM-элемента в JS, которое напрямую соответствует атрибуту `class` в HTML. Оно представляет все CSS-классы элемента как единую строку, разделённую пробелами. Это часть DOM API и работает во всех браузерах, включая старые.

**Как работает?**
- Чтение: возвращает строку с классами. Для `<div class="card active">` -> `element.className` вернёт `"card active"`.
- Запись: полностью заменяет строку классов. `element.className = "new-class hidden"` - старые классы удаляются, новые добавляются.
- Тип: всегда строка. Если классов нет, возвращает пустую строку `""`.
- Синхронизация: Изменения в `className` сразу отображаются в HTML и наоборот.

**Пример использования**
```javascript
const div = document.querySelector('#myDiv');

// Чтение классов
console.log(div.className); // "card active"

// Добавление класса (ручное, с пробелом)
div.className += ' hidden';

// Замена всех классов
div.className = "error"; // Теперь только "error"

// Удаление класса (ручное, с заменой)
div.className = div.className.replace("error", "").trim(); // Удалит "error", уберёт лишний пробел
```

**Отображение в браузере наглядно**
```javascript
let show = document.querySelector('#button');
undefined
show.className;
'hadsas'
```

**Сравнение с classList**
`classList` (рекомендуется для манипуляций):
    - Имеет методы `add()`, `remove()`, `toggle()`, `contains()`
    - Удобен для отдельных классов, предотвращает дубликаты и ошибки с пробелами;
    `div.classList.add('active') - добавляет класс без замены других

`className`
    - Простая строка, но требует ручной работы с пробелами (например, `+= "class"` создать дубликаты или лишние пробелы);
    - Лучше для полной замены или чтения всех классов сразу.
    - В старых браузерах (до IE10) `classList` не поддерживается, так что `className` - альтернатива.


**Когда использовать className?**
- Для полной замены классов: когда нужно сбросить всё и установить новые (например, в циклах или при переключении состояний);
- Для чтения/парсинга: если нужно обработать все классы как строку (например, с `split()` или регулярками).
- В старых браузерах: где `classList` недоступен.
- Не использовать: для частых добавлений/удалений - лучше `classList` для избежания ошибок.

**Недостатки**
- Ручная работа с пробелами: легко добавить дубликаты (например, `className += "active"` может дать `"card active active"`)
- Менее безопасно: ошибки в строке ломают стили (лишние пробелы или отсутствие пробелов)
- Неудобно для динамики: для toggle или проверки лучше `classList`.



### classList
`classList` - свойство DOM-элемента в JS, предоставляющее удобный интерфейс для работы с CSS-классами элемента. Оно представляет собой объект типа `DOMTokenList` (список токенов), содержащий все классы элемента как массивоподобную структуру. Это часть стандарта HTML5 и работает во всех современных браузерах.

**Пример использования**
```javascript
const div = document.querySelector('#myDiv');

// Добавляем классы
div.classList.add('card', 'active');

// Переключаем класс
div.classList.toggle('hidden'); // Добавит 'hidden', если его нет

// Проверяем и удаляем
if (div.classList.contains('active')) {
    div.classList.remove('active');
}

// Результат: <div id="myDiv" class="card hidden"></div>
```

**Как это работает?**
- `classList` автоматически синхронизируется с атрибутом `class` элемента. Изменения через `classList` сразу отражаются в HTML.
- Доступ: `element.classList` (`document.querySelector('.my-div').classList`).
- Классы хранятся как строки, разделённые пробелами, но `classList` позволяет манипулировать ими по отдельности.

**Основные методы**
- add
- remove
- toggle
- contains
- replace
- length
- item

**Когда использовать**
- Рекомендуется: для динамического управления классами в интерактивных приложениях (показ/скрытие элементов, анимация). Лучше ручного изменения `element.className` (строки), так как `classList` предотвращает ошибки с пробелами и дубликатами.
- Не использовать: если нужно работать со всей строкой классов (тогда `element.className`).
- Поддержка: Работает с IE10+; для старых браузеров нужно использовать полифилы.

**classList наглядно в браузере**
```javascript
let show = document.querySelector('#button');

> show.classList
< DOMTokenList(3) ['btn', 'btn-outline-primary', 'mb-3', value: 'btn btn-outline-primary mb-3']0: "btn"1: "btn-outline-primary"2: "mb-3"length: 3value: "btn btn-outline-primary mb-3"[[Prototype]]: DOMTokenList
```


**Дополнительно**
- DOM-элемент - это объект в JS, представляющий HTML-тег в документе (например, `<div>` или `<button>`). Он позволяет программно манипулировать структурой, стилем, содержимым и поведением страницы через API DOM.

- DOMTokenList - интерфейс для работы со списками разделённых пробелами значений (токенов), таких как CSS-классы. Он используется в `classList` для удобного добавления, удаления и проверки классов, обеспечивая автоматическую синхронизацию с атрибутом `class` и предотвращая ошибки с пробелами/дубликатами.

- Полифилы - это JS-библиотеки или скрипты, которые добавляют поддержку современных функций (например, `classList` или `fetch`) в старых браузерах, где они отсутствуют, эмулируя их поведение.


### function declaration и function expression

**function declaration** (можно вызывать до объявления функции)
```javascript
console.log(calculateSum(10, 20)); // 30
function calculateSum(x, y) {
    let result = x + y;
    return result;
};
console.log(calculateSum(10, 20)); // 30
```


**function expression** (нельзя вызвать до объявления функции)
```javascript
console.log(calculateSumma(10, 20)); // Ошибка: uncaught ReferenceError: Cannot access 'calculateSumma' before initialization
const calculateSumma = (x, y) => x + y;
console.log(calculateSumma(10, 20)); // 30

// Аналог записи FE
const calculate = function(x, y) {
    // Код...
}
```

### Массивы
Способ группировки данных, при котором данные объединяются в одну коллекцию. Подходят для значений, объединённых между собой по смыслу.
```javascript
const fruits = ["Яблоко", "Груша", "Слива"];
```

**Метод forEach**
```javascript
const fruits = ["Яблоко", "Груша", "Слива"];
fruits.forEach((item, index) => {
    console.log(`${index} => ${item}`);
});
// Метод forEach принимает cb-функцию с параметрами (значение элемента, индекс)
```

### var, let, const (отличия)

#### var
var имеет **функциональную область видимости**. Это значит, что они доступны в пределах всей функции, где объявлены, включая все вложенные блоки (циклы, условия), но не за пределами функции. Если `var` объявлена вне функции, она становится глобальной переменной. var был введён в неактуальном поныне стандарте ES5, в новых программах следует использовать let и const.

**Пример-1 (внутри функции)**
```javascript
function doSomething(){
    var a = 10;
    if (a) {
        var b = 20;
        let c = 30;
    }
    console.log(b); // 20 (доступна везде внутри функции)
    console.log(c); // ошибка
};

doSomething();
    console.log(b); // ошибка (вне функции b не определена)
    console.log(c); // ошибка
```

**Пример-2 (в цикле)**
```javascript
for (var i = 0; i < 3; i++){
    console.log(i); // i доступна здесь (0, 1, 2)
}
console.log(i); // 3 (значение доступно после завершения цикла, что может привести к неожиданным результатам)
```

#### let, const
let и const имеют **блочную область видимости**, что означает видимость переменных только внутри конкретного блока `{}`, включая вложенные блоки, но не снаружи.
`const` отличается от `let` тем, что её значение нельзя переприсвоить после инициализации.

**Пример-1 (внутри функции)**
```javascript
if (true) {
    const a = 10;
    const b = 20;
}
console.log(a); // Ошибка: a не определена
console.log(b); // Ошибка: b не определена
```

**Пример-2 (внутри функции)**
```javascript
if (true) {
    for (let i = 0; i < 3; i++){
        for (let j = 1; j <= 3; j++){
            console.log(i, j); // Корректный вывод
        }
        console.log(j); // Ошибка
        console.log(i); // Корректный вывод
    }
    console.log(i); // Ошибка
};
```

**Пример-3 (внутри цикла)**
```javascript
for (let j = 0; j < 3; j++){
    console.log(j); // j доступна только здесь
}
console.log(j); // Ошибка: j не определена (предотвращает утечку переменной и делает код более предсказуемым)
```

**Пример-4 (внутри вложенного блока)**
```javascript
function test(){
    var x = 1; // Функциональная видимость
    if (true) {
        let y = 2; // Блочная видимость
        console.log(x); // 1 (x доступна)
        console.log(y); // 2 (y доступна)
    }
    console.log(x); // 1 (x доступна)
    console.log(y); // Ошибка (y недоступна)
}
test();
```

#### Ключевые отличия
- Изоляция. `let/const` предотвращают случайное переопределение переменных в разных блоках (например, циклах или условиях), что снижает ошибки. `var` может перезаписывать себя в разных частях программы.
- Глобальная видимость. `var` вне функции может загрязнять глобальное пространство, `let/const` всегда ограничены блоком.
- Поднятие (hoisting). `var` поднимается в начало функции (можно использовать до объявления, но со значением `undefined`). `let/const` тоже поднимаются, но в "мёртвой зоне" (ошибка при доступе до объявления).
- Рекомендации. `let` для изменяемых переменных, `const` для констант. `var` устарел и может привести к багам в сложном коде (например, в асинхронных функциях или модулях).


#### Пример различия функциональной и блочной областей видимости
```javascript
{
    var test1 = 'test1';
    let test2 = 'asddas';
    const test3 = 'qwerty';
}

console.log(test1); // test1
console.log(test2); // ошибка
console.log(test3); // ошибка
```


### data-атрибуты (html)
Data-атрибуты (data attributes) - это специальные атрибуты в HTML, позволяющие хранить дополнительные данные прямо в элементах DOM. Они начинаются с `data-` и могут содержать любую строку (`data-user-id="123"`, `data-tooltip="Подсказка"` и т.д.). Эти атрибуты не влияют на стандартное поведение браузера или CSS, но доступны для JavaScript. Это своего рода кармашек для данных в DOM-элементах.

Дата-атрибуты созданы для хранения информации, с их помощью можно идентифицировать элементы. Они имеют непосредственное отношение к данным, скриптам, подходят для идентификации нескольких связанных объектов (например, при реализации аккордеона для выявления связанных элементов с целью повесить обработчик события по клику);

Ещё они важны для решения проблемы с изменением классов. Если класс изменится, то придётся скрипт переделывать, а вот дата-атрибут не зависит от класса, поэтому он остаётся статичным. Классы больше ориентированы на стилизацию, **в то время как data-атрибуты предназначены для JS, а не для стилей**.

data-атрибуты могут не иметь значения (и это не будет ошибкой):
```html
<div class="card p-3 hidden" data-tab-content id="tab-1">
```

**Использование в HTML**
```html
<button data-action-example="save" data-value-example="42">Отправить</button>
```

**Обработка в JS**
```javascript
const button = document.querySelector('button');
console.log(button.dataset.actionExample); // "save"
console.log(button.dataset.valueExample); // "42"
```

#### Зачем нужны data-атрибуты?
- Хранение метаданных. Позволяют прикреплять данные к элементам без изменения структуры HTML или использования глобальных переменных. Это полезно для динамического контента, где данные приходят из JS или сервера.
- Изоляция данных. Избегают смешивания логики с HTML (например, вместо хранения текста в JS, храните его в data-атрибуте для гибкости).
- Доступность и поддерживаемость. Упрощают код, делая его более декларативным. Например, вместо хардкода в JS можно читать данные из HTML, что упрощает изменения.
- SEO и валидность. Не ломают HTML-стандарты и игнорируются поисковиками/валидаторами.


#### Когда нужны data-атрибуты?
- Для динамических данных. Когда элементам нужно хранить состояние или конфигурацию (например, ID пользователя, настройка виджета).
- В интерактивных приложениях. Для тултипов, модальных окон, форм.
- При локализации или конфигурации. Хранить тексты, URL или параметры в HTML, чтобы JS обрабатывал их оттуда.
- **НЕ ИСПОЛЬЗОВАТЬ, ЕСЛИ**. Данные лучше хранить в JS (сложные объекты) или если атрибуты делают HTML перегруженным. Для больших объёмов данных предпочтительнее JSON или API.


#### dataset - принцип работы
`dataset` - свойство DOM-элемента в JavaScript (стандарт HTML5), которое предоставляет доступ ко всем data-атрибутам элемента как к объекту. Оно автоматически преобразует data-атрибуты в свойства объекта. Для data-атрибутов лучше работать с ним, чем через getAttribute (он больше подходит для других атрибутов и старых браузеров)

- `dataset` - это объект (типа `DOMStringMap`), где ключи - это имена data-атрибутов без префиксов `data-`, а значения - строки из атрибутов.

**Пример (HTML)**
```html
<div data-user-name="John" data-age="30"></div>
```

**Обработка (JS)**
```javascript
const div = document.querySelector('div');
console.log(div.dataset); // { userName: "John", age: "30" }
```

#### Важные нюансы работы dataset
- JS автоматически убирает префикс `data-` и преобразует kebab-case в camelCase (`data-open-text` => `openText`).

**Практический пример использования (открыть/скрыть блок)**
```html
    <h1 class="mb-4">Скрыть/Открыть блок</h1>

    <button class="btn btn-outline-primary mb-3" id="button" type="button" data-open-text="Открыть блок" data-close-text="Закрыть блок">
        Открыть блок
    </button>

    <div class="card card-body content-hidden" id="content">
        Блок с контентом. Lorem ipsum dolor sit amet consectetur adipisicing elit. 
        Quos amet illum molestias qui accusantium saepe doloribus voluptate modi unde, eaque dicta velit voluptatibus 
        cupiditate, maiores similique odit veritatis, animi tempora?
    </div>
```

```javascript
/* Получение значений из дата-атрибутов для кнопки */
const blockContent = document.querySelector('#content');
const btnOpen = document.querySelector('#button');

const openText = btnOpen.dataset.openText;
const closeText = btnOpen.dataset.closeText;

// const openText = btnOpen.getAttribute('data-open-text');
// const closedText = btnOpen.getAttribute('data-closed-text');

btnOpen.addEventListener('click', () => {
    blockContent.classList.toggle('content-hidden');
    btnOpen.innerText = blockContent.classList.contains('content-hidden') ? openText : closeText;
});
```

### DOM API
DOM API (Document Object Model Application Programming Interface) - это набор методов и свойств в JavaScript для программного взаимодействия с HTML/XML-документами. Позволяет создавать, изменять, удалять элементы, атрибуты, стили и обрабатывать события. Пример: `document.querySelector()`, `element.classList.add()`. Это основа для динамических веб-страниц.


### FETCH API
FETCH API - это современный интерфейс в JavaScript для выполнения HTTP-запросов (GET, POST и т.д.) к серверу. Заменяет старый XMLHttpRequest, возвращает Promise. Пример: `fetch('url').then(reponse => response.json())`. Поддерживается в современных браузерах, для старых - полифилы.

### Emmet быстрая запись (принцип)
```html
list-group-item[value="igor"][name="test"]{Test 3}*5
<!-- 
 [] указывает свойство со значениями
 {} устанавливает значение тега
-->
```

Получается
```html
    <list-group-item value="igor" name="test">Test 3</list-group-item>
    <list-group-item value="igor" name="test">Test 3</list-group-item>
    <list-group-item value="igor" name="test">Test 3</list-group-item>
    <list-group-item value="igor" name="test">Test 3</list-group-item>
    <list-group-item value="igor" name="test">Test 3</list-group-item>
```

**Если нужно инкрементировать число (c 1)**
```html
li.list-group-item[data-tab="tab-$"]{Test $}*4
```

**Инкремент с другого числа**
```html
li.list-group-item[data-tab="tab-$@5"]{Test $@5}*4
```


### getElementById (преимущества)
`getElementById`: 
1. Работает быстрее, чем `querySelector`, поскольку:
- ищет элементы только по ID (уникальному атрибуту);
- использует оптимизированный внутренний индекс DOM-элементов по ID;
- не анализирует CSS-селекторы.

2. Ясное намерение. Код явно показывает, что вы ищите элемент по идентификатору - нет двусмысленности с другими типами селекторов.

3. Надёжность:
- ID в HTML должен быть уникальным на странице;
- метод гарантированно найдёт первый найденный элемент (в теории - единственный);
- нет риска ошибиться с синтаксисом селектора (как в querySelector).

4. Поддержка браузерами. `getElementById` существует с древних версий DOM (уровень 1) и поддерживается абсолютно всеми браузерами, включая очень старые.

#### Пример деструктуризации массива
```javascript
let [name, surname] = ['Igor', 'Oshust'];
console.log(name); // 'Igor'
console.log(surname); // 'Oshust'
```