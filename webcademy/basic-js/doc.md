...
- Кастомные события `modal.dispatchEvent(new Event('modal:open'));`
- classList
- closest
- FETCH API
- DOM API
...
### closest
`closest()` - метод DOM-элементов в JS, позволяющий найти ближайший родительский элемент, соответствующий заданному CSS-селектору. Поиск начинается с самого элемента и движется вверх по DOM-дереву до корня документа.
```javascript
element.closest(selector);

element - DOM-элемент, от которого начинается поиск
selector - CSS-селектор для поиска родительского элемента
```

**Принцип работы**
Метод последовательно проверяет:
1. Сам элемент на соответствие селектору;
2. Родительский элемент;
3. Далее всех предков по цепочке.

Как только находится первый подходящий элемент - поиск прекращается и возвращается найденный элемент. Если совпадений нет - возвращается null.
...
### getElementById (преимущества)
`getElementById`: 
1. Работает быстрее, чем `querySelector`, поскольку:
- ищет элементы только по ID (уникальному атрибуту);
- использует оптимизированный внутренний индекс DOM-элементов по ID;
- не анализирует CSS-селекторы.

2. Ясное намерение. Код явно показывает, что вы ищите элемент по идентификатору - нет двусмысленности с другими типами селекторов.

3. Надёжность:
- ID в HTML должен быть уникальным на странице;
- метод гарантированно найдёт первый найденный элемент (в теории - единственный);
- нет риска ошибиться с синтаксисом селектора (как в querySelector).

4. Поддержка браузерами. `getElementById` существует с древних версий DOM (уровень 1) и поддерживается абсолютно всеми браузерами, включая очень старые.