...
Статусы:
1. pending (ожидание). Начальное и единственное состояние, в котором промис находится сразу после создания. 
Операция ещё не завершена - промис "ждёт" результата.
...
Происходит, когда внутри конструктора промиса вызывается функция `resolve(value)`. Это переводит статус из `pending` в `fulfilled`.
...
/* Пример получения данных по апи с последующей записью */

const headerFour = document.querySelector('.headerFour');
const bodyFour = document.querySelector('.bodyFour');
const idFour = document.querySelector('.idFour');

const apiResponse = fetch('https://jsonplaceholder.typicode.com/posts/1').
then(response => response.json()). // запись эквивалентна function(response) { return response.json() }
then(data => {
    idFour.innerText = data['id'];
    headerFour.innerText = data['title'];
    bodyFour.innerText = data['body'];
}).
catch(error => console.error(error)); // function(error) { console.log(error) };


/* Коллбеки
Коллбэк - функция, передаваемая в качестве аргумента другой функции и должна быть выполнена после завершения работы этой функции. 
