"""Что такое MongoDB?"""
# MongoDB - это документно-ориентированная NoSQL-БД, используемая для хранения и управления данными в формате JSON-подобных документов.
...
# 1. Документная модель. Данные храняться в виде документов, которые представляют собой структуры данных в формате BSON (Binary JSON). Это позволяет хранить сложные и вложенные данные.
# 2. Гибкость схемы. В отличие от реляционных БД, монго не требует строгой схемы. Это означает, что разные документы в одной коллекции могут иметь разные структуры.
# 3. Масштабируемость. MongoDB поддерживает горизонтальное масштабирование, что позволяет добавлять новые серверы для обработки увеличиващщегося объёма данных и нагрузки.
# 4. Высокая производительность. MongoDB оптимизирована для быстрого чтения и записи данных, что делает её подходящей для приложений с высокими требованиями к производительности.
...
# Он действует как масшрутизатор запросов и направляет их в определённые шарды, и обеспечивает распределённую обработку запросов.
...
# db.users.find({ age: {$gt: 20}, email: {$exists: false} })db. - эта запись эквивалентна вышестоящей
